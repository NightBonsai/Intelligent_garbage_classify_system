...
class UserInforAddWin(QWidget):
    ...
    def init_control(self):
        ...
        :return:
        """
        self.title = QLabel("用户信息录入界面", self)
        self.user_name_title = QLabel("用户名", self)
        self.user_name_title.setFont(QFont("楷体", 12))
        self.user_name_edit = QLineEdit(self)
        self.user_name_edit.setMaxLength(12)
        ...
        self.user_name_title.setGeometry(10, 70, 150, 50)
        self.user_name_edit.setGeometry(100, 70, 150, 50)
    ...
    def regist_judge_slot(self, face_mat):
        ...
        if self.user_name_edit.text():
            # 判断用户名是否已经存在
            search_flag = self.db.select_op("select * from tbl_userInfo where userName='{}';"
                                            .format(self.user_name_edit.text()))
            print(search_flag, len(search_flag))
            # 若用户名不存在
            if len(search_flag) == 0:
                ...
                if match_flag is False:
                    ...
                    access_token = self.ai.get_access_token("5mGMwYSVemPCN9zLbtZHlQLh",
                                                            "HT5ArjvXAfui3tvm2ifSZujQI3D8lGc4")
                    regist_flag = self.ai.face_regist(face_mat, "users", self.user_name_edit.text(), access_token)
                    if regist_flag is not False:
                        # 数据库写入新用户信息
                        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")    # 获取当前时间
                        status_flag = self.db.insert_op("insert into tbl_userInfo"
                                                        "(userName,userStatus,userIcon,registTime) "
                                                        "values('{}','0','null','{}');"
                                                        .format(self.user_name_edit.text(), current_time))
                        print("数据库写入新用户信息：{}".format(status_flag))
                        if status_flag is not False:
                            self.face_detect_frame.setPixmap(QPixmap("resources/icon/face_detect.png"))  # 重置图片
                            self.user_name_edit.setText("")                                             # 表单清空
...
if __name__ == "__main__":
    ...
    user_infor_add_win = UserInforAddWin()
    user_infor_add_win.show()
